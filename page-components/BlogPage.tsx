...
const BlogPage: React.FC<BlogPageProps> = ({ posts: blogPosts }) => {
  return (
    <div className="min-h-screen bg-[#00050d] py-24 px-6 md:px-12 lg:px-24">
      <div className="max-w-7xl mx-auto mt-16">
        <div className="text-center mb-16 animate-fade-in">
          <h1 className="text-5xl md:text-7xl font-black mb-6 text-white drop-shadow-xl">
            ORCA 4K TV <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-white">BLOG</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post) => (
            <Link
              href={`/blog/${post.slug}`}
              key={post.id}
              className="group bg-[#001a36] rounded-3xl overflow-hidden hover:scale-105 transition-all duration-300 border border-white/5 hover:border-red-500/50 shadow-2xl flex flex-col"
            >
              <div className="relative h-56 overflow-hidden bg-[#000a1c]">
                {post.imageUrl && (
                  <img
                    src={post.imageUrl}
                    alt={post.title}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#001a36] to-transparent"></div>
