...
interface GermanyVideoSectionProps {
  onScrollToPricing: () => void;
}

const GERMANY_FEATURES = [
  "Unbegrenzter Zugang zu deutschen Sendern",
  "Dedizierte IPTV Deutschland Server",
  "Sofortige Aktivierung"
];

const GermanyVideoSection: React.FC<GermanyVideoSectionProps> = ({ onScrollToPricing }) => {
  return (
      ...
      <div className="container mx-auto px-6 h-full flex flex-col lg:flex-row items-center justify-between gap-16">
        ...
        <div className="flex-1 text-center lg:text-left z-10 w-full mb-8 lg:mb-0 order-2 lg:order-1">
          ...
          <div className="flex flex-col gap-4 mb-10 items-center lg:items-start">
            {GERMANY_FEATURES.map((feature, index) => (
              <div key={index} className="flex items-center gap-3 text-white font-bold text-lg">
                <div className="w-6 h-6 rounded-full bg-red-700 flex items-center justify-center text-[10px] shadow-lg shadow-red-700/20 flex-shrink-0">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
            <button
              ...
              className="bg-red-700 hover:bg-yellow-500 hover:text-black text-white px-12 py-4 rounded-lg font-black uppercase text-sm tracking-[0.2em] shadow-lg shadow-red-700/30 transition-all transform hover:scale-105 active:scale-95 w-full sm:w-auto text-center"
        ...
        <div className="flex-1 w-full max-w-3xl relative order-1 lg:order-2">
          <div className="aspect-video bg-[#000a1c] rounded-xl overflow-hidden border-[12px] border-[#222] shadow-[0_0_100px_rgba(220,0,0,0.25)] relative group will-change-transform">
            <video
              autoPlay
              loop
              muted
          ...
          {/* Background glow effects - Red/Gold themed */}
          <div className="absolute -z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-red-700/10 blur-[100px] rounded-full pointer-events-none"></div>
          <div className="absolute -z-10 top-0 right-0 w-[80%] h-[80%] bg-yellow-500/10 blur-[100px] rounded-full pointer-events-none"></div>
        </div>
      </div>
    </section>
  );
...
export default GermanyVideoSection;
