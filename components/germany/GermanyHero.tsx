    ...
    <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 overflow-hidden bg-[#00050d] pt-40 md:pt-48 pb-20">
      ...
      <div className="relative z-10 max-w-4xl w-full px-4">
        ...
        <h1 className="text-5xl md:text-8xl lg:text-9xl font-black mb-4 leading-[0.9] tracking-tighter text-white drop-shadow-[0_20px_20px_rgba(0,0,0,0.8)]">
          BESTES<br />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 via-yellow-400 to-red-600 bg-[length:200%_auto] animate-shimmer">
            IPTV DEUTSCHLAND
          </span>
        </h1>
        ...
        <div className="max-w-2xl mx-auto mb-16">
          <form onSubmit={handleSearch} className="relative group transition-all duration-500 mb-8">
            ...
            <input
              ...
              placeholder="Frag was du willst: 'Spitzenfußball heute Abend?' · 'Motorsport am Sonntag?' · 'Pokalfinale?'"
            ...
            <button
              ...
              className="absolute right-3 top-3 bottom-3 bg-red-700 hover:bg-yellow-500 hover:text-black text-white px-4 md:px-8 rounded-full transition-all flex items-center gap-3 font-black uppercase text-xs tracking-widest disabled:opacity-50 shadow-xl active:scale-95 z-10"
            >
              {isSearching ? <i className="fas fa-circle-notch fa-spin"></i> : <i className="fas fa-search"></i>}
              <span className="inline">SUCHEN</span>
            </button>
          </form>
          ...
          {aiResponse && (
            <div className="mt-8 p-6 bg-white/5 backdrop-blur-2xl rounded-3xl border border-white/10 text-left animate-fade-in shadow-2xl mb-8">
              <div className="flex items-center gap-2 text-[10px] font-black text-yellow-400 uppercase mb-3 tracking-[0.2em]">
                <i className="fas fa-robot"></i> Deutschland Concierge
              </div>
