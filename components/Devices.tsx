...
const Devices: React.FC = () => {
  return (
    <section className="py-24 bg-[#002952]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
          {devices.map((device, idx) => (
            <div key={idx} className="flex flex-col items-center group cursor-pointer">
              <div className="w-20 h-20 rounded-2xl bg-[#001f3f] flex items-center justify-center mb-4 border border-white/5 transition-all group-hover:border-[#00E5FF] group-hover:-translate-y-2">
                <i className={`${device.icon} text-3xl text-gray-400 group-hover:text-[#00E5FF] transition-colors`}></i>
              </div>
