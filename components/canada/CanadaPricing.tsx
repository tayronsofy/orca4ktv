          ...
          {currentPlans.map((plan, idx) => (
            <div
              ...
              className={`
                ...
                ${plan.isPopular ? 'scale-105 z-20 shadow-[0_0_50px_rgba(239,68,68,0.3)]' : 'hover:scale-105 z-10'}
              `}
            >
              ...
              <div className="relative h-full bg-[#001530] rounded-[22px] p-6 flex flex-col border border-red-500/20 overflow-hidden">
                ...
                {plan.savings && (
                  <div className="absolute top-4 left-4">
                    <span className="bg-red-500/20 text-red-300 border border-red-500/30 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">
                      {plan.savings}
                    </span>
                  </div>
                )}
                ...
                <ul className="space-y-4 mb-8 flex-grow">
                  {CANADA_FEATURES.map((feature, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-gray-300">
                      <div className={`mt-0.5 min-w-[18px] h-[18px] rounded-full flex items-center justify-center ${plan.isPopular ? 'bg-red-500/20 text-red-400' : 'bg-white/10 text-gray-400'}`}>
