...
const CANADA_FEATURES = [
  "Unrestricted Canadian Channel Access",
  "Dedicated IPTV Canada Servers",
  "Instant Activation"
];

const CanadaVideoSection: React.FC<CanadaVideoSectionProps> = ({ onScrollToPricing }) => {
  return (
      ...
      <div className="container mx-auto px-6 h-full flex flex-col lg:flex-row items-center justify-between gap-16">
        ...
        <div className="flex-1 text-center lg:text-left z-10 w-full mb-8 lg:mb-0 order-2 lg:order-1">
          <h2 className="text-3xl md:text-5xl font-black mb-6 text-white leading-tight uppercase tracking-tight drop-shadow-lg">
            THE BEST IPTV CANADA PROVIDER.<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-white">
              Canadian Networks. Buffer-Free.
            </span>
          </h2>

          <p className="text-gray-300 text-lg md:text-xl font-medium mb-8 leading-relaxed max-w-2xl mx-auto lg:mx-0">
            Unlock the ultimate Canada IPTV experience. Dive into a massive 4K library of obsession-worthy Canadian series, live top-tier hockey & premium sports, and local news broadcasts nationwide.
          </p>

          <div className="flex flex-col gap-4 mb-10 items-center lg:items-start">
            {CANADA_FEATURES.map((feature, index) => (
          ...
          <div className="text-2xl md:text-3xl font-black text-white mb-10 tracking-tight uppercase">
            START STREAMING TODAY — FROM JUST $15.99/MO.
          </div>
        ...
        <div className="flex-1 w-full max-w-3xl relative order-1 lg:order-2">
          <div className="aspect-video bg-[#0a0a0a] rounded-xl overflow-hidden border-[12px] border-[#222] shadow-[0_0_100px_rgba(239,68,68,0.25)] relative group will-change-transform">
            <video
              autoPlay
              loop
              muted
          ...
          {/* Background glow effects - Red/White themed */}
          <div className="absolute -z-10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-red-600/10 blur-[100px] rounded-full pointer-events-none"></div>
          <div className="absolute -z-10 top-0 right-0 w-[80%] h-[80%] bg-white/10 blur-[100px] rounded-full pointer-events-none"></div>
        </div>
      </div>
    </section>
  );
...
export default CanadaVideoSection;
